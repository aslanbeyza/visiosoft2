// Raporlar sayfası metinleri — mevcut sayfadan alınmıştır.
export const reportsCopy = {
  seo: {
    title: 'Otopark Yazılımında Raporlar | Visiosoft',
    description:
      'Visiosoft otopark yazılımı; operasyonel, finansal ve abonelik raporlarını tek panelde sunar. Her rapor tipi detaylı analiz ve yeni rapor ekleme esnekliği sağlar.',
  },
  hero: {
    eyebrow: 'Raporlama Mimarisi',
    title: 'Otopark Yazılımında Raporlar',
    lead: 'Operasyon, finans ve abonelik verilerini tek panelde sade ve hızlı şekilde sunar.',
    highlightsLabel: 'Raporlama katmanının özellikleri',
    highlights: ['Anlık Akış', 'Esnek Filtre', 'Tek Tık Dışa Aktarım', 'Yeni Raporlara Açık'],
    primary: 'Teklif Al',
    secondary: 'Rapor gruplarını inceleyin',
    reportUnit: 'rapor',
  },
  subNavLabel: 'Rapor kategorileri',
  newReportsNav: 'Yeni Rapor',
  finance: {
    imageAlt: 'Zone finansal rapor ekranı: toplam geçiş, ciro, tahsilat ve borç özet kartları',
    caption: 'Zone finansal rapor ekranı (demo verisi)',
  },
  newReports: {
    anchor: 'yeni-raporlar',
    eyebrow: 'Yeni Rapor Ekleme',
    title: 'Yeni rapor eklemek kolaydır.',
    lead: 'Sistem modülerdir. Yeni raporlar, filtreler ve çıktı formatları (Excel/PDF) hızlıca eklenebilir.',
    formats: ['Excel', 'PDF'],
    formatsLabel: 'Çıktı formatları',
    items: [
      { title: 'Modüler Rapor Şablonları', description: 'Yeni raporlar mevcut yapıya hızlıca eklenir.' },
      { title: 'Esnek Filtreleme', description: 'Tarih, kapı, bariyer ve ödeme tipi filtreleri kolayca tanımlanır.' },
      { title: 'Ölçeklenebilir Veri Yapısı', description: 'Artan veri hacminde performansını korur.' },
    ],
  },
  faq: {
    anchor: 'sss',
    eyebrow: 'SSS',
    title: 'Sık Sorulan Sorular',
    label: 'Raporlar hakkında sık sorulan sorular',
    items: [
      {
        question: 'Standart raporlar dışında farklı bir rapor talebim olması durumunda ek bir ücret yansıtılıyor mu?',
        answer:
          'Mümkün olan rapor talepleri ücretsiz hazırlanabilir. Sadece size özel geliştirme gerektiren raporlar veya özellikler ayrıca ücretlendirilir.',
      },
    ],
  },
  cta: {
    eyebrow: 'Demo',
    title: 'Raporları canlı ortamda görmek ister misiniz?',
    description: 'Demo’da tüm raporları gerçek senaryolarla birlikte hızlıca inceleyebilirsiniz.',
    primary: 'Teklif Al',
    secondary: 'WhatsApp ile Ulaş',
    whatsappMessage: 'Merhaba, otopark raporlama modülü hakkında bilgi almak istiyorum.',
  },
}
