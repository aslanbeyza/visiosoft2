...
export default function HomeIntro() {
  ...
  const facts = [
    { id: 'founded', value: company.foundedYear, from: company.foundedYear - 12, grouping: false, label: text.facts.founded },
    {
      id: 'locations',
      value: company.locations.length,
      label: text.facts.locations,
      detail: company.locations.map((location) => location.label).join(' · '),
    },
    { id: 'hardware', value: hardwareMenu.length, label: text.facts.hardware },
  ]
  ...
  return (
    <Section tone="paper" spacing="lg" labelledBy="home-intro-title">
      ...
      <Reveal className={styles.factsBar} amount={0.4}>
        <dl className={styles.facts} aria-label={text.factsLabel}>
          {facts.map((fact) => (
            <div key={fact.id} className={styles.fact}>
              <dt className={styles.factLabel}>{fact.label}</dt>
              <dd className={styles.factValue}>
                <CountUp value={fact.value} from={fact.from} grouping={fact.grouping} />
              </dd>
              {fact.detail ? <dd className={styles.factDetail}>{fact.detail}</dd> : null}
            </div>
          ))}
        </dl>
        <Link to={path('team')} className={styles.aboutLink}>
          {text.aboutLink}
          <svg
            viewBox="0 0 24 24"
